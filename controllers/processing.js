    ...
    Object.keys(answerObjects)
        .map(ansKey => {
            answerChartData.push({
                'name': ansKey, 'value': answerObjects[ansKey].counter,
...
const processFile = (filepath='resources/epa-http.json') => {
    ...
    pipeline.on('end', () => {
        ...
        answerSizeChartData.push({'totalCodeOK': answerChartData[0].value, 'totalSmallSize': answerChartData[0].size});
...
const getChartData = () => {
    console.log("from within" + requestMethodChartData);
    return { requestMethodChartData,
         answerChartData,
         answerSizeChartData,
         requestsPerMinuteChartData
